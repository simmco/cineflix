...
import { FETCH_MOVIES_NOW, FETCH_MOVIES_YEAR, FETCH_MOVIES_HIGHEST_RATED, FETCH_MOVIES_MOST_POPULAR, FETCH_MOVIES_MOST_POPULAR_KIDS } from './types';
...
export function fetchMovieNow() {
  ...
  return function(dispatch) {
    axios
      .get(URL + `primary_release_date.gte=${fiveWeeksAgo}&primary_release_date.lte=${dateNow}`)
      .then(res => {
        dispatch({
...
export function fetchMovieYear() {
  ...
  return function(dispatch) {
    axios
      .get(URL + `primary_release_date.gte=${oneYearAgo}&primary_release_date.lte=${dateNow}&sort_by=vote_average.desc&vote_count.gte=1000`)
      .then(res => {
        dispatch({
...
export function fetchMovieMostPopularKids() {
  return function(dispatch) {
    axios
      .get(URL + `certification_country=US&certification.lte=G&sort_by=popularity.desc`)
      .then(res => {
        dispatch({
